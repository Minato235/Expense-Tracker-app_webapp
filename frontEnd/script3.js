...
async function addNewExpenses(e) {
  ...
  try {
    ...
    const responce = await axios.post("http://localhost:3000/expense/expenseDetails", expenseDetails, {
        headers: {
          "Authorization": token
        }
      })
...
document.getElementById("rzp").onclick = async function (e) {
  ...
  const token = localStorage.getItem("token");
  const responce = await axios.get("http://localhost:3000/user/premiumAccount", {
    headers: {
      "Authorization": token
    }
  })
  var options = {
    ...
    "handler": async function (responce) {
      await axios.post("http://localhost:3000/user/updatePremiumAccount", {
...
function showLeaderBoard() {
  ...
  input.onclick = async () => {
    ...
    const token = localStorage.getItem("token");
    const userLeaderBoard = await axios.get("http://localhost:3000/showLeaderBoard", {
      headers: {
        "Authorization": token
      }
    })
...
function paginationFunc(pageNo){
  ...
  axios.get(`http://localhost:3000/expense/expenseDetails?pageNo=${pageNo}&items_per_page=${items_per_page}`,{
